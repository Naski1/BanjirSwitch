...
export default function PetugasDashboard() {
    ...
    async function loadData() {
        setLoading(true);
        try {
            const [reportsData, zonesData] = await Promise.allSettled([
                getFieldReports(),
        ...
        <div className="dashboard-page">
            ...
            <div className="dash-header">
                <div className="dash-header-info">
                    ...
                    <div>
                        <h1 className="dash-title">Dashboard Petugas</h1>
                        <p className="dash-subtitle">
                            Selamat datang, <strong>{user?.full_name || 'Petugas'}</strong>
                            {user?.village_name && <> — Wilayah: {user.village_name}</>}
                        </p>
                    </div>
                </div>
            ...
            <div className="tabs" style={{ marginTop: 24 }}>
                {[
                    { key: 'overview', label: '📋 Laporan Saya' },
                    { key: 'create', label: '➕ Input Bencana' },
                    { key: 'create_shelter', label: '🏠 Input Shelter' },
                    { key: 'zones', label: '🗺️ Zona Aktif' },
                ].map(t => (
                    <button key={t.key} className={`tab ${activeTab === t.key ? 'active' : ''}`}
                        onClick={() => { setActiveTab(t.key); if (t.key.startsWith('create')) setShowForm(true); }}>
                        {t.label}
                    </button>
                ))}
            </div>
